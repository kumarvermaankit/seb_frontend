...
const Register = () => {
  ...
  const { name, email, password, cf_password, pic ,c_mail} = userData;
  ...
  const handleSubmit = async () => {
    setIsLoading(true);
    const message = valid(name, email, password, cf_password,c_mail);
    ...
    try {
      ...
      const { data } = await axios.post(
        "/api/users",
        { name, email, password, pic ,c_mail},
        config
      );
      toast.info("User Registered Successfully");
      localStorage.setItem("sebUser", JSON.stringify(data));
      setIsLoading(false);
      navigate("/chats");
    } catch (error) {
      toast.error(error);
